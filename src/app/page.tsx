...
async function getEvents(): Promise<EventModel[]> {
  return (await fetch('http://localhost:3000/api/events')).json()
}
      ...
      <div className="grid grid-cols-3 gap-8">
        {events.map((event,key) => (
          <div key={key} className="bg-white shadow-lg rounded-lg">
            <img className="w-full h-48 object-cover" src={event.image_url} alt={event.name} />
            <div className="p-4">
              ...
              <p className="text-gray-500 mt-2"> R$ {event.price}</p>
              <p>
